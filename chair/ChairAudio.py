...
def logWrite(Heading, Orientation):
	...
	f = open('logFile', 'a')
	f.write("%s" % n + '\t' + Heading + '\t' + Orientation + '\n')
	f.flush()
	f.close()
...
while True:
	...
	timeSinceSpeak += time.time() - lastTime
	if pygame.mixer.music.get_busy():
		timeSinceSpeak = 0
	
	if audioMode == "past":
		determineAudio(data, lastDir, pastContent)
		
	elif audioMode == "present":
		determineAudio(data, lastDir, presentContent)
		
	elif audioMode == "event":
		determineAudio(data, lastDir, eventContent)
	
	lastTime = time.time()
	
	# print(timeSinceSpeak)
	# if timeSinceSpeak > 10:
	#	playIdleAudio()
	#	nextIdleSpeak += 1
	#	
	#	if nextIdleSpeak > 2:
